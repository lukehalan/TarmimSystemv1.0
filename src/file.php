<?php 
	...
	if( isset( $_GET["date"] ) && isset( $_GET["file"] ) ){
		...
		$path = DOWNLOAD_PATH . "/$date";
		$file = base64_decode( $_GET["file"] );
		
		if( is_object( $rowAccount ) ){
			
			if( $rowAccount->block == "yes" ){
				
				$errorArray["download"] = "حساب کاربری شما مسدود شده است !";
				
			}
			else if( !strstr( $rowAccount->active_date , $date . ";" ) ){
				
				$errorArray["download"] = "شما اجازه دانلود فایل مورد نظر را ندارید !";
				
			}
			else{
				
				sendFileStream( $path , $file );
				exit();
...
<body>
	...
	<table align="center" border="0" cellpadding="0" cellspacing="0" width="750">
		...
		<tr>	
			<td id="main"><?php include("includes/pages/file.php"); ?></td>
		</tr>
